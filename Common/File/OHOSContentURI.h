...
public:
	OHOSContentURI() {}
	explicit OHOSContentURI(std::string_view path) {
		Parse(path);
	}

	bool Parse(std::string_view path);

	OHOSContentURI WithRootFilePath(const std::string &filePath);
	OHOSContentURI WithComponent(std::string_view filePath);
	OHOSContentURI WithExtraExtension(std::string_view extension);  // The ext string contains the dot.
	OHOSContentURI WithReplacedExtension(const std::string &oldExtension, const std::string &newExtension) const;
	OHOSContentURI WithReplacedExtension(const std::string &newExtension) const;

	bool CanNavigateUp() const;

	// Only goes downwards in hierarchies. No ".." will ever be generated.
	bool ComputePathTo(const OHOSContentURI &other, std::string &path) const;
	...
	bool TreeContains(const OHOSContentURI &fileURI) {
		if (root.empty()) {
			return false;
		}
