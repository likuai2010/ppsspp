...
VkResult VulkanContext::CreateInstance(const CreateInfo &info) {
    ...
    instance_extensions_enabled_.push_back(VK_OHOS_SURFACE_EXTENSION_NAME);
#else
...
#endif
#if defined(__OHOS__)
    case WINDOWSYSTEM_OHOS: {
        VkSurfaceKHR surface;
        VkSurfaceCreateInfoOHOS surfaceCreateInfo = {};
        surfaceCreateInfo.sType = VK_STRUCTURE_TYPE_SURFACE_CREATE_INFO_OHOS;
        surfaceCreateInfo.window = (OHNativeWindow*)winsysData1_; // 这里的nativeWindow就是从上一步骤OnSurfaceCreatedCB回调函数中拿到的
        retval = vkCreateSurfaceOHOS(instance_, &surfaceCreateInfo, nullptr, &surface_);
        break;
    }
#endif
#if defined(VK_USE_PLATFORM_METAL_EXT)
